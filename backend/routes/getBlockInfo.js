...
router.post('/', function (req, res, next) {
  ...
  var parameters = {
    //"blockNum": 1
    "blockNum": Number(req.body.blockNum)
  };
  ...
  var json = JSON.stringify({
    ...
    console.log("getBlockInfo_publish");
    ...
    var interval = setInterval(function() {
      req.cache.hexists("resChannel", uid, function(err, exists) {
        console.log("getBlockInfor_hexists: " + exists);

        if(exists) {
          req.cache.hget("resChannel", uid, function(err, obj) {
            console.log("getBlockInfo_hget");
            if(err) {
                console.log(err);
            }
